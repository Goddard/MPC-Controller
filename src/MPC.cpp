...
public:
  ...
  void operator()(ADvector &fg, const ADvector &vars)
  {
    ...
    for (int t = 1; t < N; t++)
    {
      ...
      // x_t = x[t-1] + v[t-1] * cos(psi[t-1]) * dt
      // y_t = y[t-1] + v[t-1] * sin(psi[t-1]) * dt
      // psi_t = psi[t-1] + v[t-1] / Lf * delta[t-1] * dt
      // v_t = v[t-1] + a[t-1] * dt
      // cte_t = f(x[t-1]) - y[t-1] + v[t-1] * sin(epsi[t-1]) * dt
      // epsi_t = psi[t] - psides[t-1] + v[t-1] * delta[t-1] / Lf * dt
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
  ...
  const int degrees = 25;
  const float delta_limit = ( degrees * M_PI ) / 180;
